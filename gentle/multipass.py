...
def prepare_multipass(alignment):
    ...
    for _, wd in enumerate(alignment):
...
def realign(wavfile, alignment, ms, resources, nthreads=4, progress_cb=None):
    ...
    def realign_sub(chunk):
        ...
        k = standard_kaldi.Kaldi(
            resources.nnet_gpu_path, chunk_gen_hclg_filename, resources.proto_langdir
        )
    ...
    def safe_realign_sub(*args):
        try:
            realign_sub(*args)
        except Exception:
            logging.warning("multipass:pool: %s", exc_info=True)

    with ThreadPool(nthreads) as pool:
        pool.map(safe_realign_sub, to_realign)
